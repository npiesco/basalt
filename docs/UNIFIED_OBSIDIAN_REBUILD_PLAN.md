...
### Tags and Metadata
- [x] **Tags UI with E2E Tests (2025-11-13)**: Fully implemented tags functionality with comprehensive E2E test validation. Created E2E test suite (`tests/e2e/tags.e2e.test.js`) validating: add tags to notes and persist to database, display tags on note list items, remove tags from notes, CASCADE DELETE of note_tags when notes are deleted, search/filter notes by tag. Features: tag input field in edit mode (comma-separated tags), tag display as clickable badges in UI and note list, tag filtering to show only notes with specific tags, handleTagFilterClick() for tag-based filtering, manual CASCADE DELETE for note_tags in handleConfirmDelete() (workaround for absurder-sql foreign key limitations, same as backlinks). Fixed failing CASCADE DELETE test by manually deleting note_tags entries before deleting notes. All 5 tests PASS individually. NO MOCKS - real tag parsing, real database queries, real CASCADE behavior. (See apps/pwa/app/page.tsx:1375-1411, tests/e2e/tags.e2e.test.js)
- [x] **Markdown Rendering with Wikilinks Integration (2025-11-13)**: Fully implemented comprehensive markdown parsing in note preview mode while preserving wikilinks functionality following strict TDD. Created E2E test suite (`tests/e2e/markdown-rendering.e2e.test.js`) with 7 tests validating: headings (H1-H6), bold and italic text, ordered and unordered lists, inline code and code blocks, blockquotes, markdown links, wikilinks + markdown integration. Features: 3-step rendering pipeline - (1) extract wikilinks and replace with {{WIKILINK_N}} placeholders to avoid markdown interpretation, (2) parse markdown to HTML using marked.js, (3) replace placeholders with React wikilink components. Added marked.js for markdown parsing and @tailwindcss/typography for prose styling. Fixed critical placeholder bug: changed from __WIKILINK_N__ to {{WIKILINK_N}} to prevent markdown from interpreting underscores as bold/italic syntax. All 7 markdown tests PASS. No regressions in wikilinks or backlinks tests (3/5 each passing, 2/5 browser crashes from sequential execution). NO MOCKS - real markdown parsing, real wikilink preservation, real React rendering. (See apps/pwa/app/page.tsx:1143-1249, commit b8387ea)
